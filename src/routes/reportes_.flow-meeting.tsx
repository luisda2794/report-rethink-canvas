...
const REQUIRED_FIELDS: EpodField[] = ["waybill", "fecha", "estado", "incidencia", "cp", "driver", "tipoEntrega"];
      ...
      .then((record) => {
        ...
        const parsed: RawRow[] = record.rows.map((r) => {
          ...
          return {
            ...
            fecha,
            estado: r.estado,
            categoria: categorizar(r.estado),
  ...
  return (
    <div className="flex flex-col gap-6 print:bg-white">
      <div className="print:px-6 print:py-4">
        ...
        {/* Selector de Hub (oculto al imprimir) */}
        <div className="print:hidden mb-6">
          <label className="text-xs text-muted-foreground">Hub</label>
          <HubCombobox value={hub} onChange={setHub} className="mt-1 max-w-xs" />
          {loading && <p className="mt-2 text-xs text-muted-foreground">Cargando datos del hub…</p>}
          {error && (
